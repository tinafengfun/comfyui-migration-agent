...
5. Search local roots before remote sources, then try approved candidates in priority order. Prefer the reusable migration asset tool pool when available so provider search/download/clone state is captured as compact JSON instead of long chat context:
   - **cross-task asset-resolution ledger** (`/nfs_share/migration-knowledge/asset-resolutions.jsonl` by default) — the backend already checks this automatically for every row before local/provider search runs; if a prior task resolved this exact asset name and the file still exists, it's reused directly (no re-search) and the row shows `resolved_ledger_reuse` / a `source` value starting with `ledger reuse:`. You don't need to query this file yourself — just don't re-search or re-report a gap for a row already marked this way.
...
7. If the remaining hard stop is source-known but not staged, run a bounded acquisition pass before asking for human input: copy/download exact model files into an isolated workflow cache that mirrors both ComfyUI's model layout and the custom node's expected cache layout, or record why policy/access/exactness blocks the item.
   ...
   Note (clarification, does not change the acquisition rules above): when staging a downloaded model, place it directly inside the ComfyUI folder convention its loader scans — `diffusion_models/`, `text_encoders/`, `audio_encoders/`, `loras/`, `vae/` (and the analogous subfolders of any configured `extra_model_paths.yaml` root) — rather than at the model root. ComfyUI's folder scanner only walks these named subdirectories; a file staged at the root (e.g. `/nfs_share/<model>.safetensors` instead of `/nfs_share/diffusion_models/<model>.safetensors`) will not appear in any loader's dropdown and later steps (e.g. Step 05) would have to compensate by creating symlinks into the correct subfolder. Staging into the right subfolder up front avoids that repair.
...
8a. **Implicit package dependencies (enum widget values).** Read `00-enum-dependencies.csv` from Step 00. Each row is an enum widget value (e.g. `sampler_name="res_2s"`, `scheduler="bong_tangent"`) that exists in the SOURCE environment but not in target core — meaning a source-side custom package (e.g. RES4LYF) injected it into a core node's dropdown. Treat each as a custom-node acquisition item exactly like a missing node: for `state=source known`, the `resolving_package` names the repo — search/verify/clone it into the workflow cache (do NOT substitute the value). For `state=source unknown`, identify the providing package from the source environment (which package registers that sampler/scheduler); if it cannot be identified, that is a hard stop for human input. Never resolve an enum-value dependency by changing the value — that breaks apple-to-apple fidelity (see agent.md rule 3a).
