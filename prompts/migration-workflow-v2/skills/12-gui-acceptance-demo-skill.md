...
2. Prepare a clean ComfyUI environment recipe:
   ...
   - **Honor `artifacts/effective-run-config.json`** (written by the Step 07/08 capacity ladder + reduced-tier decision). Copy its `vram_flags` verbatim into the delivery/handoff launch script so the customer environment starts the same way. **If `reduced_tier` is `true`, run the GUI acceptance at `recommended_reduced_setting` (lower resolution/frames)** — this is the operator-approved fidelity tier that avoids the full-size OOM; record the pass as reduced-tier, NOT full-size customer-ready. Do NOT relaunch the server with different/default flags. See [capacity-vram-mitigation-ladder](capacity-vram-mitigation-ladder.md).
