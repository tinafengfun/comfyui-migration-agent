...
2. Prepare a clean ComfyUI environment recipe:
   ...
   - **If `reduced_tier` is `true`, the reduction has ALREADY been applied for you — use the pre-generated reduced workflow, do NOT re-derive it.** The backend wrote `reduced_prompt_path` (`artifacts/reduced-runtime-policy-prompt.json`) by deterministically applying `recommended_reduced_setting.changes` (the exact node edits: width 720→480, height 1280→832, length 81→49, frame_load_cap 121→49) to the runtime-policy prompt. Base the GUI demo workflow on THAT reduced prompt (convert it to the GUI copy the tester loads). **Verify** the reduction is present before asking the operator to accept: the resolution/frame widgets show the reduced values, and a smoke run's attention `seq=`/token count is far below the full-size figure that OOM'd (a `seq=155440` or ~6 min/sampling-step means you're still on the full-size graph — you used the wrong file). If `reduced_prompt_path` is missing, fall back to applying `recommended_reduced_setting.changes` yourself and verify the same way. Record the pass as reduced-tier, NOT full-size customer-ready. See [capacity-vram-mitigation-ladder](capacity-vram-mitigation-ladder.md). For WAN2.2-class video workflows (dual fp8 diffusion + VLM), see [wan22-capacity-reference](wan22-capacity-reference.md) — frames drive `seq` more than resolution, and Step 12 launches at the persisted `vram_flags` (the backend now hardens the previous step's BKC, so there is no lowvram→novram drift).
