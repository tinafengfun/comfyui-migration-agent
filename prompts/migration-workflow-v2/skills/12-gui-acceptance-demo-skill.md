...
2. Prepare a clean ComfyUI environment recipe:
   ...
   - **If `reduced_tier` is `true`, you MUST actually apply the reduced settings to the GUI workflow the tester runs — do not hand them the full-size graph.** `recommended_reduced_setting.changes` is an explicit list of exact node edits (each `{node_id, input, old, new}`, e.g. width 720→480, height 1280→832, length 81→49, frame_load_cap 121→49). Apply every one of them to the demo workflow copy, then **verify** the reduction took: the resolution/frame widgets show the new values, and a smoke run's attention sequence length / token count is far below the full-size figure that OOM'd. A demo that still runs at full-size resolution (e.g. a huge `seq=...` in the log, or ~6 min/sampling-step) means the reduction was NOT applied — fix it before asking the operator to accept. Record the pass as reduced-tier, NOT full-size customer-ready. See [capacity-vram-mitigation-ladder](capacity-vram-mitigation-ladder.md).
