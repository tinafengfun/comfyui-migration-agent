...
2. Prepare a clean ComfyUI environment recipe:
   ...
   - **DO NOT launch your own ComfyUI.** The backend has already started the ONE correct server for this step: the **validated docker container + NFS venv** (the same environment Steps 05–08 used — it has `llama_cpp`, the fp8 keep-on-move patch, and the effective VRAM flags), reachable at the node's api_url. Never start a bare-metal / local-checkout ComfyUI (that environment is missing `llama_cpp` and will hang the VLM node — real incident 2026-08-11). Verify the backend-launched server is up (`/system_stats`, `/object_info`) and use it as-is; if it's not reachable, report an environment gap rather than improvising a launch.
   - **Honor `artifacts/effective-run-config.json`** (written by the Step 07/08 capacity ladder + reduced-tier decision). Copy its `vram_flags` verbatim into the delivery/handoff launch script so the customer environment starts the same way. Do NOT relaunch the server with different/default flags.
   - **If `reduced_tier` is `true`, the reduction has ALREADY been applied for you — use the pre-generated reduced workflow, do NOT re-derive it.** The backend wrote `reduced_prompt_path` (`artifacts/reduced-runtime-policy-prompt.json`) by deterministically applying THIS workflow's `recommended_reduced_setting.changes` (the exact per-node edits vary by workflow — e.g. for a WAN2.2-class video graph they might be width 720→480, height 1280→832, length 81→49, frame_load_cap 121→49; an image workflow has no frame widgets at all) to the runtime-policy prompt. Base the GUI demo workflow on THAT reduced prompt (convert it to the GUI copy the tester loads). **Verify** the reduction is present before asking the operator to accept: the reduced widgets from `recommended_reduced_setting.changes` show their new values, and a smoke run's attention `seq=`/token count is far below the full-size figure that OOM'd (still seeing the full-size `seq`/a many-minutes-per-sampling-step — e.g. a WAN2.2-class full-size `seq≈155440` — means you're still on the full-size graph and used the wrong file). If `reduced_prompt_path` is missing, fall back to applying `recommended_reduced_setting.changes` yourself and verify the same way. Record the pass as reduced-tier, NOT full-size customer-ready. See [capacity-vram-mitigation-ladder](capacity-vram-mitigation-ladder.md). For WAN2.2-class video workflows (dual fp8 diffusion + VLM), see [wan22-capacity-reference](wan22-capacity-reference.md) — frames drive `seq` more than resolution, and Step 12 launches at the persisted `vram_flags` (the backend now hardens the previous step's BKC, so there is no lowvram→novram drift).
