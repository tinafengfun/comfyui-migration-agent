...
**Reduced-size "acceptance tier" (hand this to Step 12).** When full-size hits any of the above capacity failures, record `capacity_tier: "reduced"` in `step12_context` (or the completion decision) with a concrete recommended setting that fits — halve spatial dims and/or frames until the token count drops well under the failing point (e.g. 480×832 × 49 was confirmed to run clean end-to-end where 720×1280 × 81 failed). This is the reliable GUI-acceptance path; the full-size "customer-ready" claim stays a capacity hard stop needing a larger/multi-GPU node. Step 12 runs its GUI acceptance at this reduced tier and downgrades the claim boundary accordingly.
