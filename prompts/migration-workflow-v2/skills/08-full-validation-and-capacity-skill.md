...
Reasonable mitigations include targeted CPU placement for VAE/text/image preprocess stages, reserve adjustment, validated attention mode changes, reduced frame count/resolution for a restricted tier, or multi-XPU escalation. Repeating generic `lowvram` settings *by hand, open-endedly,* without a new hypothesis is not a mitigation — but note the **system now owns a bounded, deterministic lossless VRAM ladder** (`--lowvram` → `--novram`, each tried once) that runs automatically on a capacity OOM before you are asked for the reduced tier. See [capacity-vram-mitigation-ladder](capacity-vram-mitigation-ladder.md). Do NOT hand-toggle lowvram/reserve-vram yourself; write your summary and let the orchestrator escalate or gate.
...
**The reduced setting is now VALIDATED, not just recommended.** When the capacity-probe recommends a reduced tier, `step08_full_validation.py` runs a **second, cheap `reduced-validation` probe** that actually applies the reduced changes (frames + resolution) and runs the graph once, so `step12_context.reduced_validation.validated` records whether the reduced config really clears OOM before Step 12 commits to it. The operator's capacity gate surfaces this verdict. If `validated` is `false`, reduce **frames first** (they drive the attention `seq` more than resolution) or escalate the VRAM tier. For WAN2.2-class video (dual fp8 diffusion HIGH/LOW + UMT5-XXL TE + a CPU VLM), see [wan22-capacity-reference](wan22-capacity-reference.md) for the model/VRAM budget, the frames-vs-resolution finding, and why offload never fixes the FP16 activation overflow.

**When the full-size probe DEVICE_LOSTs, the inline validation is DEFERRED — this is expected, not a failure.** A `DEVICE_LOST`/OOM at full size kills the XPU handle (the HTTP server stays up but can't run anything), so an inline reduced-validation would hang STUCK PENDING and — critically — could leave the summary unwritten, which historically caused Step 12 to silently ship the FULL-SIZE workflow and OOM (real incident 2026-08-13, task 051acd0a). The tool now detects this: it **skips the inline probe** and sets `reduced_validation.needs_reset = true`, but **always still writes the structured `recommended_reduced_setting.changes`** so Step 12 gets the correct reduced config regardless. To empirically confirm the reduced config after a deferral: **reset the XPU** (the backend does this automatically on the capacity-retry relaunch, or run `xpu-smi config -d 0 --reset` after stopping the container) and run the tool standalone with **`--run-level reduced-validation`** on the clean server — it applies the reduced changes and reports `reduced_validation.validated`. Never accept the reduced tier without `recommended_reduced_setting.changes` present: the backend now **hard-stops** Step 08 if the reduced tier is accepted with no structured changes (that would push the full-size graph to Step 12).
