...
7. **Human gates:** every gate must be Web-visible and include background/reason/scene, terminology explanations, choices, consequences/follow-up, continuation edge, and claim-boundary impact. Vague gates like "approve?" or "provide direction" are invalid.
...
12. **task-state.json schema:** every step must update `task-state.json` with the full schema specified in this contract (steps array with completion_decision, claim_boundary sections, compaction refs, human_decisions list). Do not commit schema drift — the flat-object form (without steps array, without claim_boundary, without compaction refs) is deprecated. **Never hand-edit `task-state.json`'s raw JSON text.** A real run corrupted the file exactly this way: the terminal step's completion entry landed outside the `steps` array with an orphaned bracket, because completing the last step requires touching both the array's last element and top-level fields after it in one edit — a riskier splice than a normal step's in-place object replace. Instead: write this step's completion object to a scratch JSON file (a fresh file write is safe; a splice into a large existing file is not), then call `npx tsx scripts/patch-task-state.mts --artifacts <artifactPath> --step-file <scratch.json> [--top-level-file <scratch-toplevel.json>]` — this is the only path that guarantees valid JSON, auto-repairs a previously corrupted file if it finds one, and refuses to silently proceed on unparseable content it doesn't recognize.
